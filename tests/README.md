...
Four fully-isolated regression tests for the v0.8.2 hardening that landed in v3.0.0 and the release-manager lane.
...
| `test_release_manager.sh` | Uses a fake `gh` binary to verify release-manager dry-run safety, ready-PR filtering, apply command shape, no premature `release:merged` label without merge evidence, deploy workflow polling, and live lock behavior. |
